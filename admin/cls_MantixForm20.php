    ...
    function ver() {
        ...
        if(count($this->botones)>0 && ($_POST["accion"]==20 || $_POST["accion"]==2)) {
            ...
            foreach($this->botones as $btn) {
                ...
                $scb.="    f.idobjfk.value=f.idobj.value;"."\n";
                $scb.="    f.submit();"."\n";
                $scb.="})"."\n";
            }
            $scb.='</script>';
        ...
        if(isset($_POST["quiensoy"])) {
			...
			<input name="Enviar" type="button" class="form_submit" value="Guardar y Enviar" onClick="validar1()" style="margin-left:1px">';
        }
        $cadf.="\r\n";
        $cadf.='<input type="hidden" name="idobjfk" value="'.$_REQUEST["idobjfk"].'" />';
