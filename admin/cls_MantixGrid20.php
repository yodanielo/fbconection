...
class MantixGrid {
    ...
    function mostrar_buscador() {
        if($this->atributos["ver_buscador"]=="1") {
            ...
            for($a=0; $a<=count($this->buscador)-1;$a++) {
                ...
                switch($tipo) {
                    ...
                    case "select":
                        ...
                        break;
                    case "fecha":
                        $r.= '<input size="10" id="bus_' .$this->buscador[$a]["id"].'" class="form_date" type="text" name="bus_' .$this->buscador[$a]["id"].'" value="'.$_POST['bus_'.$this->buscador[$a]["id"]].'" title="DD/MM/YYYY" />'."\n";
            ...
            //$r.='<script type="text/javascript">$(".field_buscador").slideDown(1000, function(){});</script>';
        }
        return $r;
    }
                ...
                if($_POST['bus_'.$this->buscador[$a]["id"]]!="") {
                    ...
                    switch($tipo) {
                        ...
                        case "select" : $cad.= $this->buscador[$a]["campo"]."='".$_POST['bus_'.$this->buscador[$a]["id"]]."'";
                            break;
                        case "fecha":
                            $cad.=$this->buscador[$a]["campo"]."='".preg_replace("/([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})/","$3-$2-$1",$_POST['bus_'.$this->buscador[$a]["id"]])."'";
                            break;
                    }
                }
            }
    ...
    function ver() {
        ...
        if($this->atributos["ver_buscador"]=="1") {
            if($this->atributos["sql"]!="" and $cad!="") {
                $pwhere = strpos($this->atributos["sql"], "where");
                if($pwhere===false) {
                    $this->atributos["sql"].=" where ".$cad;
                }
                else {
                    $this->atributos["sql"]=  str_replace("where","where $cad and ",$this->atributos["sql"]);
                }
            }
        ...
        if(!$_POST["tipo"]) {
            ...
            $r.='<input type="hidden" id="idobjfk" name="idobjfk" value="'.$_REQUEST["idobjfk"].'">';
		...
		for ($a=1;$a<=$this->contador_esp;$a++)
		{
            ...
            if(!$_POST["tipo"]) {
                ...
                if ($t_pag>1 ) {
                    ...
                    $r.="".$patras.'P&aacute;gina: <input type="text" value="'.$p_actual.'" name="pag" id="irpag" /> de <span id="tpag">'.$t_pag.'</span>';
                    if ($p_actual <$t_pag ) {
            ...
            $r.='<br><br><div align="center" class="grid_paginado" style="clear:both">No existen registros para mostrar.</div>';
        }
        if(!$_POST["tipo"]) {
            $this->datos->cerrar();
