...
{
 $this->ini_datos("fbc_useradmins","usuario");
}
...
function ingresar()
{
	$this->datos->listaSP("select * from fbc_useradmins where usuario='".str_replace("'","",trim($_POST["txt_user"]))."' and password='".md5(trim($_POST["txt_pass"]))."'","","");
...
if($_SESSION["user"]["password"]==trim(md5($_POST["password"]))) {
	$this->datos->ejecutar("update fbc_useradmins set password='".md5(trim($_POST["nueva"]))."' where id=".$_SESSION["user"]["id"]);
...
{
$r = new MantixGrid();
$r->atributos=array("tabla"=>"fbc_useradmins","nropag"=>"20","ordenar"=>"usuario");
$r->columnas=array(
