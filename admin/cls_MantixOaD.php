...
class Columna {
    ...
    var $marcado;
    function formato() {
        $this->valor=mysql_escape_string($this->valor);
        switch ($this->tipo) {
...
//        $this->db =  mysql_select_db("ftm", $this->con);
        $this->con = mysql_connect("localhost","reymundo_root","JwTX0D3TT0ol") or die(mysql_error());
        $this->db =  mysql_select_db("reymundo_fbconexion", $this->con);
    }

    function ejecutar($sql,$r=FALSE) {
        if($r)
