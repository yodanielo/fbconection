...
<div id="cmcontent">
    <form id="frmcomu" action="#" method="post">
        ...
        <div id="contbotones">
            <input type="submit" id="btnsendmessage" value="<?= __("btnsendmessage") ?>" />
    ...
    <?php
    if ($params["hayrango"])
        echo '<div class="msgrango">' . $params["hayrango"] . '</div>';
    if (count($params["posts"]) == 7)
        echo '<div class="msgrango">' . __("txtlimitposts") . '</div>';
    ?>
        ...
        if (count($params["posts"]) > 0) {
            ...
            foreach ($params["posts"] as $p) {
                ...
                <div class="postitem' . ($p->enviado != 1 ? ' postenviado' : '') . '" id="post' . $p->id . '">
                    ...
                    <div class="posttext">
                        <div class="postusername"><a target="_blank" href="' . $usuarios["f" . $p->user_id]["profile_url"] . '">' . htmlspecialchars($usuarios["f" . $p->user_id]["name"]) . '</a></div>
                        <div>' . htmlspecialchars($p->mensaje) . '</div>';
                if ($p->attachThumb != "")
                    $cad.='<div class="attacheditem"><div class="aititle"><strong>' . $p->attachTitle . '</strong></div><div class="ailink"><a target="_blank" href="' . str_replace("javascript:", "", $p->attachURL) . '">http://' . htmlspecialchars($p->attachURL) . '</a></div></div>';
                $cad.='        <div class="postlinks"><span class="postdate">' . htmlspecialchars($p->fecha2) . '</span></div>
                    </div>
                </div>
                ';
                /* if ($p->enviado == 1)
        ...
        if(urlin.split(" ").join("")!=""){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $("#btnattach").fadeOut(450, function(){});
